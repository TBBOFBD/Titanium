...
pub fn handle_input_events() {
    ...
    if !KEYBOARD_BINDS.lock().unwrap().is_empty() {
        set_hook(WH_KEYBOARD_LL, &*KEYBOARD_HHOOK, keybd_proc);
    };
...
unsafe extern "system" fn keybd_proc(code: c_int, w_param: WPARAM, l_param: LPARAM) -> LRESULT {
    if KEYBOARD_BINDS.lock().unwrap().is_empty() {
        unset_hook(&*KEYBOARD_HHOOK);
    } else if w_param as u32 == WM_KEYDOWN || w_param as u32 == WM_SYSKEYDOWN {
        if let Some(bind) = KEYBOARD_BINDS
            .lock()
            .unwrap()
            .get_mut(&Keyboard::from(u64::from(
...
fn send_keybd_input(flags: u32, key_code: Keyboard) {
